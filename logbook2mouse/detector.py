...
def measurement(experiment, duration: float = 1.0, store_location: Path = Path(".")):
    ...
    det_status = epics.caget(f"{experiment.eiger_prefix}:State_RBV")
    ...
    sleep(.1)
    is_triggered = epics.caget(f"{experiment.eiger_prefix}:Acquire_RBV")
    ...
    pattern = epics.caget(f"{experiment.eiger_prefix}:FWOutputFilePrefix").split("_")[0]
